package ComputerSection;

import net.jodah.failsafe.internal.util.Assert;
import org.apache.poi.xssf.usermodel.XSSFSheet;
import org.apache.poi.xssf.usermodel.XSSFWorkbook;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.WebDriverWait;

import java.io.FileInputStream;
import java.io.FileNotFoundException;
import java.io.IOException;
import java.time.Duration;

public class AddItems {
    WebDriver driver;
    WebDriverWait wait;
    By clickOnComputerOnUpperMenuBar = By.xpath("//a[contains(text(),'Computers ')][1]");
    By clickOnNoteBooks = By.xpath("//a[contains(text(),' Notebooks ')]");
    By verifySixItemsAreShown = By.xpath("//div[@class='page-title']");
    By addToCartButton = By.xpath("(//button[contains(text(),'Add to cart')])[2]");
    By goToTheShoppingCart = By.xpath("//span[contains(text(),'Shopping cart')]");
    By verificationOfAddToCart = By.xpath("(//a[contains(text(),'Asus N551JK-XO076H Laptop')])[2]");
    By clickOnCheckBox = By.xpath("(//input[@type='checkbox'])[2]");
    By clickOnCheckOutButton = By.xpath("(//button[contains(text(),' Checkout ')])");
    By clickOnRegisterButton = By.xpath("//button[@class='button-1 register-button']");
    By enterFirstName = By.xpath("  //input[@name='FirstName']");
    By enterLastName = By.xpath("//input[@name='LastName']");
    By emailInput = By.xpath("//input[@name='Email']");
    By enterPassword = By.xpath("//input[@name='Password']");
    By enterConfirmedPassword = By.xpath("//input[@name='ConfirmPassword']");
    By enterRegisterButton = By.xpath("  //button[contains(text(),'Register')]");
    By clickOnContinueButton = By.xpath("//a[contains(text(),'Continue')]");
    By clickOnCheck = By.xpath(" //input[@name='termsofservice']");
    By buttonClickOnCheckOut = By.xpath(" //button[@name='checkout']");
    By enterFirstNameOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.FirstName']");
    By enterLastNameOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.LastName']");
    By enterEmailOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.Email']");
    By enterCountryOnShippingDetails = By.xpath("//select[@name='BillingNewAddress.CountryId']");
    By enterCityOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.City']");
    By enterAddressOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.Address1']");
    By enterZipCodeOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.ZipPostalCode']");
    By enterPhoneNumberOnShippingDetails = By.xpath("//input[@name='BillingNewAddress.PhoneNumber']");
    By enterContinueButtonOnShippingDetails = By.xpath("//button[@onclick='Billing.save()']");
    By enterContinue2ButtonOnShippingDetails = By.xpath("//button[@onclick='ShippingMethod.save()']");
    By enterContinue3ButtonOnShippingDetails = By.xpath("//button[@onclick='PaymentMethod.save()']");
    By verifyTheDetailsAreAdded = By.xpath("//span[@class='sku-number']");


    public AddItems(WebDriver driver) {
        this.driver = driver;
        wait = new WebDriverWait(driver, Duration.ofSeconds(20));

    }

    public void verifyComputer() throws IOException, InterruptedException

    {
        driver.findElement(clickOnComputerOnUpperMenuBar).click();
        driver.findElement(clickOnNoteBooks).click();
        wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath("//div[@class='page-title']"))));
        String actual = driver.findElement(verifySixItemsAreShown).getText();
        Assert.isTrue(actual.equals("Notebooks"), "Expected result does not match with actual result");
        driver.findElement(addToCartButton).click();
        driver.findElement(goToTheShoppingCart).click();
        wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.xpath("(//a[contains(text(),'Asus N551JK-XO076H Laptop')])[2]"))));
        actual = driver.findElement(verificationOfAddToCart).getText();
        Assert.isTrue(actual.equals("Asus N551JK-XO076H Laptop"), "Expected result does not match with actual result");
        driver.findElement(clickOnCheckBox).click();
        driver.findElement(clickOnCheckOutButton).click();
        driver.findElement(clickOnRegisterButton).click();
                String path = System.getProperty("user.dir") + "//src//test//java//TestData//RegisterationData.xlsx";
        FileInputStream prop1 = null;
        try
        {
            prop1 = new FileInputStream(path);
        }
        catch (FileNotFoundException e)
        {
            e.printStackTrace();
        }
        XSSFWorkbook wb = new XSSFWorkbook(prop1);
        XSSFSheet sheet = wb.getSheet("Sheet1");
        String firstName = sheet.getRow(1).getCell(0).getStringCellValue();
        String lastName = sheet.getRow(1).getCell(1).getStringCellValue();
        String email = sheet.getRow(1).getCell(2).getStringCellValue();
        String password = sheet.getRow(1).getCell(3).getStringCellValue();
        String confirmedPassword = sheet.getRow(1).getCell(4).getStringCellValue();
        driver.findElement(enterFirstName).clear();
        driver.findElement(enterLastName).clear();
        driver.findElement(emailInput).clear();
        driver.findElement(enterPassword).clear();
        driver.findElement(enterConfirmedPassword).clear();
        driver.findElement(enterFirstName).sendKeys("Jas");
        driver.findElement(enterLastName).sendKeys("Kaur");
        driver.findElement(emailInput).sendKeys("yy@gmail.com");
        driver.findElement(enterPassword).sendKeys("password");
        driver.findElement(enterConfirmedPassword).sendKeys("password");
        driver.findElement(enterRegisterButton).click();
        driver.findElement(clickOnContinueButton).click();
        driver.findElement(clickOnCheck).click();
        driver.findElement(buttonClickOnCheckOut).click();
        driver.findElement(enterFirstNameOnShippingDetails).sendKeys("ss");
        driver.findElement(enterLastNameOnShippingDetails).sendKeys("kaur");
        driver.findElement(enterEmailOnShippingDetails).sendKeys("yy@gmail.com");
        driver.findElement(enterCountryOnShippingDetails).click();
        driver.findElement(By.xpath("(//option[contains(text(),'India')])[2]")).click();
        driver.findElement(enterCityOnShippingDetails).sendKeys("Chandigarh");
        driver.findElement(enterAddressOnShippingDetails).sendKeys("Mohali");
        driver.findElement(enterZipCodeOnShippingDetails).sendKeys("0011");
        driver.findElement(enterPhoneNumberOnShippingDetails).sendKeys("9898982323");
        driver.findElement(enterContinueButtonOnShippingDetails).click();
        driver.findElement(enterContinue2ButtonOnShippingDetails).click();
        driver.findElement(enterContinue3ButtonOnShippingDetails).click();
        actual = driver.findElement(verifyTheDetailsAreAdded).getText();
        Assert.isTrue(actual.equals("AS_551_LP"), "Expected result does not match with actual result");
    }
}